 ...
 *
 * Zeroing the indices rather than adopting whatever is already in the region is
 * deliberate, and is what makes a driver reload safe.  The shmem regions are
 * reserved memory that survives both rmmod/insmod and an RPU restart, so stale
 * indices from a previous session would desynchronise the two sides.  The
 * firmware resets the same six indices on its side for the same reason: both
 * peers starting from 0 is the agreed handshake.  It is safe because the
 * firmware holds no cached copies -- it re-reads head and tail from the region
 * on every operation -- so it simply observes an empty ring.
 *
...
/*
 * The remote raised an IPI.  Runs in IRQ context, so only the wakeups happen
...
static void plat_mailbox_rx_callback(struct mbox_client *cl, void *data)
{
	...
	if (!mb->rx_stopped) {
		...
		/*
		 * Only drain when the RPU FW has actually queued a response.
		 * Pure doorbell-completion IPIs leave the mgmt ring empty, so
		 * scheduling rx_work for them just adds a needless wakeup on
		 * the submit fast path.  SPSC-safe: the RPU FW publishes head
		 * with a write barrier before raising the IPI, and tail is
		 * host-owned so the cached copy is authoritative.  Pair the
		 * remote's write barrier before reading its head.
		 */
		if (mb->rx_hdr) {
			dma_rmb();
...
/*
 * Raise the TX IPI to tell the remote that a ring was written.
 *
				     ...
				     const struct xdna_mailbox_res *res)
{
	...
	/* Id 0 is reserved for firmware-initiated messages. */
	mb->next_msg_id = 1;
	...
	/*
	 * The doorbell region needs room for a whole u32 slot past the header,
	 * not merely one byte: a smaller region rounds down to zero slots and
	 * would leave the ring permanently full.
	 */
	if (mb->mgmt_size / 2 <= sizeof(struct shmem_ring_hdr) ||
	 ...
	 * interrupt the remote can raise already finds initialised indices.
	 */
	plat_rings_init(mb);
...
/*
 * Consume responses the RPU has already produced.  Firmware completes some
			  ...
			  const struct xdna_mailbox_msg *msg, u64 tx_timeout)
{
	...
	/*
	 * The RPU FW copies the ring in u32 strides and rejects a payload that
	 * is not a multiple of 4 without consuming the record, which would
	 * stall its side of the ring.  Catch it here instead.
	 */
	...
	/* Allocate a non-zero id: 0 is reserved for firmware-initiated messages. */
...
unlock:
	...
	if (ret) {
		 ...
		 * because the remote reads it directly and completes some
...
/*
 * Notify firmware that @hw_ctx_id has work queued, by producing the id into the
...
int amdxdna_mailbox_plat_ring_doorbell(struct mailbox *mb, u32 hw_ctx_id)
{
	...
	/*
	 * If the ring is full, wait for the remote to consume rather than
	 * dropping the doorbell: a dropped kick is not recoverable from here,
	 * because the firmware only re-examines a context when it sees a
	 * doorbell for it.  Every RX IPI means the firmware has run its
	 * doorbell ISR, so db_waitq is woken on the same event that makes room.
	 * db_lock must be released while sleeping.
	 */
	...
	if (ret) {
		XDNA_ERR(mb->xdna, "Doorbell produce failed for hw_ctx %u, ret %d",
			 hw_ctx_id, ret);
		goto unlock;
	}
