 ...
 * preserved - or a negative errno if firmware did not confirm the destroy.
 *
 * A negative errno does not mean the context survived.  Firmware keeps tearing
 * a context down even when it reports an error, and resets the AIE4 if that is
 * what it takes, so the caller treats the ctx as gone on every reply.  Most
 * errors are parameter checks - an id that does not exist or belongs to another
 * VF - which leave nothing behind to retry.
 *
...
/*
 * Driver suspend gracefully destroys the in-flight hwctx so that the
 * subsequent resume can bring those hwctx back properly.
 *
 * The graceful flag asks firmware to clean up an in-flight hwctx gracefully:
 * stop it at a preemption point rather than mid-job, and save its state so a
 * later create can pick it up. It handles the following scenarios:
 *
 * No | firmware reply           | destroy returns | suspend        | resume
 * ---|--------------------------|-----------------|----------------|---------------------------
 * 1  | SUCCESS, restore_id != 0 | restore id      | save ctx state | restore ctx, resume jobs
 * 2  | NO_RESTORE (status 0x4)  | 0               | abort jobs     | new ctx, jobs were aborted
 * 3  | ERROR (status 0x1)       | -EINVAL         | log error      | new ctx, restart jobs
 * 4  | no reply (timeout)       | -ETIME          | log error      | new ctx, restart jobs
 *
 * Row 1: firmware returns SUCCESS with a nonzero restore id when it has
 * preempted the ctx and saved the ctx state. The state goes into the firmware
 * context restore pool. A firmware suspend wipes that pool, so the driver
 * copies the pool into a buffer of its own before the power transition
 * happens. On resume the driver writes back the restore pool to firmware
 * before creating any ctx. The create ctx request from the driver passes the
 * restore id to firmware, which then restores the state from the pool, and
 * the in-flight job carries on from where it was preempted.
 *
 * Row 2: firmware returns NO_RESTORE when it cannot preempt the ctx, so it
 * never reaches a point where the state can be saved. NO_RESTORE means
 * firmware destroyed the ctx without preserving it, so its parked jobs can no
 * longer be re-driven and hence suspend aborts them. The resume then builds an
 * empty ctx. Aborting sets each command to ERT_CMD_STATE_ABORT, so user space
 * gets its commands back instead of waiting on them, and has to resubmit.
 *
 * Rows 3 and 4: firmware returns ERROR, or does not reply in time. Neither
 * saves any state, but the ctx is still gone: firmware keeps destroying a ctx
 * even when it reports an error. Nothing can stop the suspend here anyway -
 * aie4_hwctx_destroy() returns void, so the failure never reaches it - and the
 * power transition wipes firmware state regardless. The jobs stay queued, and
 * resume builds a new ctx and runs them again from the start. This is what the
 * driver did before graceful destroy and is kept unchanged; escalating
 * repeated destroy errors to a device reset is left to a later change.
 *
 * All four rows are graceful destroys; a non-graceful destroy never asks for a
 * restore id and never reports NO_RESTORE.
 *
 * On resume, a failed create ctx is handled the same way on every row. The
 * create already cleared the restore id, so it is not used again.
 * aie4_hwctx_resume_all() stops at that ctx and returns the error, so some ctx
 * are created and some are not. The resume callback, aie4_vf_resume() or
 * aie4_classic_resume(), then destroys every ctx as AIE4_HWCTX_ERROR, drains
 * their jobs so user space gets its commands back, stops the hardware and
 * fails the resume. A failed reset does the same.
 */
